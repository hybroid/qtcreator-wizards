...
OPENCM3_DIR := $(wildcard $(LIBPATH:=/locm3.sublime-project))
...
bin: $(OBJDIR)/$(BINARY).bin
hex: $(OBJDIR)/$(BINARY).hex
...
$(OBJDIR)/%.elf: $(OBJDIR) $(OBJS) $(LDSCRIPT)
	@printf "  LD      $(*).elf\n"
...
clean:
	@printf "  CLEAN\n"
	$(Q)$(RM) $(OBJDIR)/*.elf $(OBJDIR)/*.bin $(OBJDIR)/*.hex \
			  $(OBJDIR)/*.srec $(OBJDIR)/*.list $(OBJDIR)/*.map \
			  generated.* \
			  ${OBJS} ${OBJS:%.o=%.d}

%.stlink-flash: $(OBJDIR)/%.bin
	@printf "  FLASH  $<\n"
